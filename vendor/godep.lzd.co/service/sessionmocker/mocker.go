...
func init() {
	config.RegisterBool("force_sessions_mock_enabled", "Path to sessions mock", false)
	config.RegisterString("sessions_mock_path", "Path to sessions mock", "")
	config.RegisterUint("sessions_mock_store_days", "How long store mock in days", 1)
}

func InitSessionMockerFromFlags() error {
	mockEnabled, _ = config.GetBool("force_sessions_mock_enabled")
	mockPath, _ = config.GetString("sessions_mock_path")
	storeDays, _ = config.GetUint("sessions_mock_store_days")
