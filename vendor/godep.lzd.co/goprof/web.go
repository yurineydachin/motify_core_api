...
func toggleProfiling(w http.ResponseWriter, r *http.Request) {
	...
	if enableParam != "0" && enableParam != "1" {
		...
		fmt.Fprintf(w, "Bad value for mandatory 'enable' param: '%v'. Please, use 0 or 1.", enableParam)
		return
	}
	...
	if err != nil {
		renderPage(w, fmt.Sprintf("Failed to toggle profiling (enable=%v): %v", enableProfiling, err))
		return
	}
	...
	if dir == "" {
		renderPage(w, "Seems profiling already stopped")
		return
	}
	renderPage(w, "")
}
...
func downloadProfile(w http.ResponseWriter, r *http.Request) {
	...
	if profilesDir == "" {
		w.WriteHeader(http.StatusBadRequest)
		fmt.Fprint(w, "No such profile (param 'path' is mandatory)")
		return
	}
	...
	if ourCurrentProfile != nil && ourCurrentProfile.Dir == profilesDir {
		w.WriteHeader(http.StatusBadRequest)
		w.Header().Add("Content-Type", "text/html")
		renderPage(w, "We write the requested profile at the moment. Stop it first, then you will be able to download it")
		return
	}
	...
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		fmt.Fprintf(w, "Cannot stat '%v': %v", profilesDir, err)
		return
	}
	if !fileInfo.IsDir() {
		w.WriteHeader(http.StatusBadRequest)
		fmt.Fprintf(w, "Expecting '%v' to be a directory, but it is not", profilesDir)
		return
	}
	...
	if err != nil {
		...
		fmt.Fprintf(w, "Failed to pack profiles: %v", err)
		return
	}
	_, err = io.Copy(w, archive)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		fmt.Fprintf(w, "Failed serve archive: %v", err)
		return
	}
}
