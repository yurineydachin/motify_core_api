...
type V1Args struct {
	...
	GoogleToken *string `key:"token" description:"Google token"`
}

type V1Res struct {
	Token string `json:"token" description:"Authorized token"`
	Res   string `json:"res" description:"res"`
...
func (handler *Handler) V1(ctx context.Context, opts *V1Args, apiToken token.INullToken) (*V1Res, error) {
	logger.Debug(ctx, "User/Social/GoogleLogin/V1")
	...
	GoogleUrl, _ := oauth2.GetCodeUrl(oauth2.GoogleConf)
	logger.Error(ctx, "Google url for code: %s", GoogleUrl)

	if opts.Code == nil && opts.GoogleToken == nil {
		return nil, v1Errors.MISSED_REQUIRED_FIELDS
	}

	profile, err := oauth2.GetGoogleUser(opts.Code, opts.GoogleToken)
	if err != nil {
		oauthErrors := []error{
