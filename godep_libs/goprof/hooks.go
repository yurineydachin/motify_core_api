...
type prof struct {
	Prof     profName      // which profile is this related to
	Dir      string        // directory where profiles will be placed
	Start    time.Time     // profile start time
	Duration time.Duration // how long did profile writing lasted, zero if profile is one-off
}
