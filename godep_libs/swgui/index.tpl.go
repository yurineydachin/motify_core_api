  ...
  <script type="text/javascript">
    $(function () {
      ...
      */
      ...
      if ("{{ .AppName }}") {
       window.swaggerUi.api.clientAuthorizations.add("lzd-app-name", new SwaggerClient.ApiKeyAuthorization("X-LZD-App-Name", "{{ .AppName }}", "header"));
      }
      if ("{{ .AppVersion }}") {
       window.swaggerUi.api.clientAuthorizations.add("lzd-app-version", new SwaggerClient.ApiKeyAuthorization("X-LZD-App-Version", "{{ .AppVersion }}", "header"));
      }
      if ("{{ .Node }}") {
       window.swaggerUi.api.clientAuthorizations.add("lzd-node", new SwaggerClient.ApiKeyAuthorization("X-LZD-Node", "{{ .Node }}", "header"));
      }
    ...
    function toggleDebug(button) {
      if ($(button).data('enabled')) {
           window.swaggerUi.api.clientAuthorizations.remove("lzd-debug");
           $(button).text('Enable debug').data('enabled', 0);
      } else {
           window.swaggerUi.api.clientAuthorizations.add("lzd-debug", new SwaggerClient.ApiKeyAuthorization("X-LZD-Debug", "1", "header"));
           $(button).text('Disable debug').data('enabled', 1);
      }
...
{{if .DebugAvailable }}<button onclick="toggleDebug(this)">Enable debug</button>{{end}}
